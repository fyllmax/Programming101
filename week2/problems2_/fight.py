    ...
    def simulate_fight(self):
        ...
        while True:
            ...
            if attack_turn < 50:
                ...
                print("%s attacks for %s" % (self.player1.name, damage))
                self.player2.take_damage(damage)

            else:
                damage = self.player2.attack()
                print("%s attacks for %s" % (self.player2.name, damage))
                self.player1.take_damage(damage)

            if self.player1.is_alive() is False:
                print("{} slapped badly {}".format(self.player1_name, self.player1_name))
                break
